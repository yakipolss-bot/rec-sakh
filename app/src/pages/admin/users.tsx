...
import type { AdminUser } from '@/data/adminMock';

const roles = ['Все', 'user', 'moderator', 'editor', 'admin'];
const statuses = ['Все', 'active', 'blocked', 'pending'];
const cities = ['Все', ...new Set(adminUsers.map(u => u.city))];
          ...
          <tbody>
            {paged.map((user, i) => (
              ...
              >
                ...
                <td className="py-3 px-3 text-[var(--text-secondary)] font-mono text-xs">{user.registeredAt}</td>
                ...
                <td className="py-3 px-3">
                  <div className="flex items-center gap-1">
                    <button className="sakh-btn sakh-btn--ghost sakh-btn--sm" title="Редактировать"><Eye size={14} /></button>
                    <button className="sakh-btn sakh-btn--ghost sakh-btn--sm" title="Заблокировать"><Ban size={14} /></button>
                    <button className="sakh-btn sakh-btn--ghost sakh-btn--sm" title="Удалить"><Trash2 size={14} /></button>
                    <Link to={`/admin/users/${user.id}`} className="sakh-btn sakh-btn--ghost sakh-btn--sm" title="Открыть"><ChevronRight size={14} /></Link>
                  </div>
                </td>
              </motion.tr>
            ))}
