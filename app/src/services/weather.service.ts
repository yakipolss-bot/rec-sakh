...
export const SAKHALIN_CITIES: SakhalinCity[] = [
...
export async function fetchCityForecast(cityCode: string): Promise<ForecastDay[]> {
  ...
  try {
    ...
    if (!cache) {
      const allData = await fetchAllCitiesWeather();
